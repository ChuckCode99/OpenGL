...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        projection  = glm::perspective(glm::radians(45.0f), (float)WINDOW_DEFAULT_WIDTH / (float)WINDOW_DEFAULT_HEIGHT, 0.1f, 100.0f);
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            model = i%3 ? glm::rotate(model, (float)glfwGetTime() * glm::radians(angle * (float)std::pow(-1, i)), glm::vec3(0.5f * (i + 1), 1.0f, (float)i)) \
                             : model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
            unsigned int cudeLoc = glGetUniformLocation(Program.ID, "model");
            glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
