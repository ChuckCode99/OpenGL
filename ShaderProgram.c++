...
ShaderProgram::ShaderProgram(std::string_view VertexShaderFile, std::string_view FragmentShaderFile)
	: m_ShaderProgramID{ glCreateProgram() }, ID{m_ShaderProgramID},
	  ...
	  FragmentShaderObject(Shader::ShaderType::FRAGMENT, FragmentShaderFile)
{
	Create();
}
...
void ShaderProgram::Create()
{
	...
	glAttachShader(m_ShaderProgramID, VertexShaderObject.m_ShaderID);
	glAttachShader(m_ShaderProgramID, FragmentShaderObject.m_ShaderID);
...
void ShaderProgram::setBool(const unsigned int programID, std::string_view name, bool value) const
{
	glUniform1i(glGetUniformLocation(programID, name.data()), (int)value);
}

void ShaderProgram::setInt(const unsigned int programID, std::string_view name, int value) const
{
	glUniform1i(glGetUniformLocation(programID, name.data()), value);
}

void ShaderProgram::setFloat(const unsigned int programID, std::string_view name, float value) const
{
	glUniform1f(glGetUniformLocation(programID, name.data()), value);
}

void ShaderProgram::setVec3(const unsigned int programID, std::string_view name, const glm::vec3& value) const
{
	glUniform3fv(glGetUniformLocation(programID, name.data()), 1, glm::value_ptr(value));
}

void ShaderProgram::setMat4(const unsigned int programID, std::string_view name, const glm::mat4& value) const
{
	glUniformMatrix4fv(glGetUniformLocation(programID, name.data()), 1, GL_FALSE, &value[0][0]);
}

ShaderProgram::Shader::Shader(ShaderType Type, std::string_view ShaderDataPath)
	: m_ShaderType{ Type }
{
